...
func NewHandler(swimmerService service.SwimmerService, sessionService service.SessionService) *Handler {
	swimmerService = swimmerService
	sessionService = sessionService
	return &Handler{}
}

// FindAllSwimmers retrieves all swimmers and writes them as JSON.
func (h *Handler) FindAllSwimmers(w http.ResponseWriter, r *http.Request) {
	swimmers, err := swimmerService.ListSwimmers()
	if err != nil {
...
// FindSwimmerByID retrieves a swimmer by ID.
func (h *Handler) FindSwimmerByID(w http.ResponseWriter, r *http.Request) {
	...
	swimmer, err := swimmerService.GetSwimmerByID(id)
	if err != nil {
...
func (h *Handler) AddSwimmer(w http.ResponseWriter, r *http.Request) {
	...
	swimmer, err := swimmerService.AddSwimmer(request.Name, request.Age)
	if err != nil {
...
func (h *Handler) AddSession(w http.ResponseWriter, r *http.Request) {
	...
	session, err := sessionService.AddSession(request.SwimmerID, parsedDate, request.Distance, request.Duration, request.Intensity, request.Style, request.Notes)
	if err != nil {
